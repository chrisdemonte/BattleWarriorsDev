...
public class StatementToTheUser {
	...
	public StatementToTheUser(int width, int height, Pane root) {
		this.width = width;
		this.height = height;
		this.root = root;
		this.generateLayout();
	...
	private void generateLayout() {
		...
		text.setMinSize((width * 2)/3, (height * 3)/4);
		text.setMaxSize((width * 2)/3, (height * 3)/4);
		text.setAlignment(Pos.CENTER_LEFT);
		text.setWrapText(true);
		text.setStyle("-fx-text-fill: BLACK; -fx-font-size: 16;");
		
		this.messageStatement();
		this.setButtons();
		
		messageContainer.getChildren().addAll(text, howToPlayButton, futureButton, backButton);
	...
	private void setButtons() {
		...
		backButton.setOnAction(e->{
			StartMenu menu = new StartMenu(width, height, root);
			root.getChildren().clear();
			root.getChildren().add(menu.getContainer());
		});
