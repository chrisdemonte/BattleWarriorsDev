...
public class FullAttack extends Move{
	...
	public void makeMove(Player self, Player target, BattleScene scene) {
		...
		if (this.isAttackUsable(self, target, scene.getBattleLog())) {
			this.useResources(self, target, scene.getBattleLog());
			double mod = 1.0;
			...
			if (targetStats.isOutOfReach() && (!selfStats.isReach() && !this.rangedAttack)) {
				makeContact = false;
				scene.getBattleLog().getLog().add(self.getName() + "'s " + this.getName() + " missed. " + target.getName() + " is out of range!");
				scene.getBattleLog().setLogLength(scene.getBattleLog().getLogLength() + 1);
			}
			if (makeContact && (rand.nextInt(100) > ((selfStats.getAccuracy() * selfStats.getAccuracyMod() * 100) + this.accuracy))) {
				
				makeContact = false;
				scene.getBattleLog().getLog().add(self.getName() + "'s " + this.getName() + " missed.");
				scene.getBattleLog().setLogLength(scene.getBattleLog().getLogLength() + 1);
			}
			if (makeContact && (rand.nextInt(100) < ((targetStats.getAvoidance() * targetStats.getAvoidanceMod() * 100) - (this.avoidability + (selfStats.getSkill()/4350.0))))
				...
				makeContact = false;
				scene.getBattleLog().getLog().add(self.getName() + "'s " + this.getName() + " was dodged.");
				scene.getBattleLog().setLogLength(scene.getBattleLog().getLogLength() + 1);
			}
			//damage
			if (makeContact) {
				...
				this.applyBuffs(self, target, this, scene);
				
				if (selfStats.isSelfHarm()) {
					int selfHarm = (int)((totalPhysical + totalMagic) * .5); 
					selfStats.setCurrentHealth(selfStats.getCurrentHealth() - selfHarm );
				}
				...
				for (int i = 0; i < logDetails.size(); i++) {
					scene.getBattleLog().addToLog(logDetails.get(i));
				}
				if (this.physicalPower > 0.0 || this.magicPower > 0.0) {
					scene.getBattleLog().addToLog(logEntry);
				}
			}
