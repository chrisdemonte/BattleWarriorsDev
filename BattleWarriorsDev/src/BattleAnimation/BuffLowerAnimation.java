...
public class BuffLowerAnimation extends BuffAnimation{


	private static final long serialVersionUID = 6270784413746097935L;

	public BuffLowerAnimation(String key) {
		super(key);
	}
				...
				if (!target.isNPC()) {
					timeline.getKeyFrames().addAll(
							...
							new KeyFrame(
									...
									finalHandler,
									new KeyValue(imageView.translateYProperty(), playerStartPosition[1] + scene.getPlayerSpriteContainer().getHeight())));
					timeline.play();
					...
					timeline.getKeyFrames().addAll(	
							...
							new KeyFrame(
									Duration.millis(1000),
									finalHandler,
									new KeyValue(imageView.translateYProperty(), enemyStartPosition[1] + scene.getPlayerSpriteContainer().getHeight())));	
